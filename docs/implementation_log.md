...
- section chunk 검색 구조는 chunk 수를 줄이는 장점이 있었지만, 모든 세부 후보가 같은 chunk 번호로 묶여 근거 세밀도가 떨어졌다.
...
- v0.3.3의 실제 체감 품질을 75~85점 기준선으로 둔다.
- 이후 개선은 한 PDF에 맞춘 규칙 추가가 아니라, 여러 문서/질문 평가표를 먼저 만든 뒤 진행한다.

## 2026-06-06 RAG 출력 템플릿 범용화

변경 내용:

- RAG 표 출력에서 예산안 전용 컬럼을 제거했다.
- 표 기본 컬럼을 `항목 | 핵심 내용 | 근거 파일명 | 근거 chunk`로 바꿨다.
- 질문이 금액이나 수치를 요구할 때만 `금액/수치` 컬럼을 추가한다.
- 누락 방지 후보 표도 `후보 내용 | 근거 파일명 | 근거 chunk` 기준으로 바꿨다.
- 누락 방지 후보는 `모두`, `전체`, `빠짐없이`처럼 완전성 요구가 있을 때만 붙인다.
- RAG 컨텍스트에서 page 번호 노출을 제거해 chunk 번호와 혼동될 가능성을 줄였다.

검증:

- 논문형 표 질문 프롬프트에 예산/증액/금액 전용 지시가 들어가지 않는지 확인
- 금액형 표 질문에서만 `금액/수치` 컬럼 안내가 들어가는지 확인
