...
- `scripts/create-curated-state.js` 추가
- 대화로 생성한 불안정한 예시 대신 사람이 검수한 형태의 좋은 예시 50개 구성
- 생성 시 `data/app-state.json`의 dataset을 `source: "curated"` 50개로 교체
- 이전 실패 기록은 유지하고, 프로필 대화 기록은 비워 프롬프트 오염을 줄임

설계 결정:

- 작은 로컬 모델이 계속 관련 없는 예시를 섞고 있어, 모델에게 데이터 생성을 맡기기보다 직접 작성한 기준 데이터로 전환한다.
- 이번 50개는 파인튜닝 실험 전 말투와 답변 길이 기준을 안정시키기 위한 1차 큐레이션 세트다.
