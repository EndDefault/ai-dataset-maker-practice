...
- [ ] RAG 답변에서 요청 주제와 무관한 항목 제외
- [ ] 표 요청 시 Markdown 표 형식 강제
- [ ] 벡터 검색 결과에 키워드 포함 가중치 반영
- [ ] LLM 응답이 빈 문자열이면 성공 처리하지 않기
- [ ] 빈 응답 재시도 또는 실패 기록 처리
- [ ] 근거 파일명과 chunk 정보를 답변에 더 명확히 표시
- [ ] 예산안 항목/지원 내용/금액 추출 출력 안정화
