...
최종 목표는 목적별 LoRA를 만들기 위한 학습 데이터 제작, 검수, export, 학습 비교 도구를 만드는 것이다.

현재 앱은 모델을 바로 완성시키는 도구가 아니다. 프롬프트와 템플릿을 바탕으로 학습 데이터 후보를 만들고, 검수한 뒤 JSONL로 내보내며, 작은 LoRA 학습 실험까지 연결하는 준비 도구다.
...
- `docs/planning/lora-generator-direction.md`: 템플릿 기반 LoRA 데이터셋 생성기 방향 기획
...
- `ai-maker-dataset.jsonl` 50개 기본 구조 검증
- `training/validate_jsonl.py`로 JSONL 검사 가능
- `Qwen/Qwen2.5-0.5B-Instruct` 기준 LoRA 20 step 학습 실행
- `training/output/qwen2.5-0.5b-nabi-lora/adapter_model.safetensors` 생성 확인
- 원본 모델과 LoRA adapter를 붙인 모델의 답변 차이 비교 확인
...
- 원하는 말투를 모델 자체에 더 가깝게 반영하려면 LoRA adapter가 필요하고, 1차 실험에서 adapter 생성까지 확인했다.
- LoRA는 지능을 크게 올리는 도구라기보다 말투, 형식, 반응 방식 같은 작업 습관을 조정하는 데 더 적합하다.
- 다음 방향은 하나의 말투 데이터셋을 넘어서, 템플릿 기반으로 여러 목적의 LoRA 데이터셋을 만드는 구조다.

## 현재 단계

현재 프로젝트는 `LoRA 1차 실험 확인 완료, 템플릿 기반 LoRA 생성기 기획` 단계다.
...
→ Qwen2.5 0.5B LoRA 20 step 학습
→ adapter 생성과 학습 전후 비교 확인
```
...
프로젝트별 JSONL export
목적별 데이터 100개 이상 제작
LoRA 결과 관리와 Ollama 등록 흐름 정리
```

## 다음에 할 일

추천 다음 단계는 LoRA 생성기 기획을 데이터 모델과 UI 흐름으로 구체화하는 것이다.

- LoRA 프로젝트 구조 설계
- 템플릿 구조 설계
- 데이터 후보 상태값 설계: `draft`, `auto_checked`, `reviewed`, `rejected`
- 생성 AI, 수정 AI, 평가 AI 역할 프롬프트 설계
- 코드 검사 규칙 설계
- 프로젝트별 JSONL export 설계
...
데이터를 더 늘릴 때는 하나의 말투 데이터만 늘리는 것이 아니라, LoRA 프로젝트별 목적과 템플릿에 맞춰 확장한다.
...
지금 만든 것은 AI를 바로 완성시킨 결과물이 아니라, 사람이 통제한 기준과 템플릿을 바탕으로 목적별 LoRA 데이터셋을 만들고 검수하며 학습 실험까지 이어가기 위한 로컬 AI 데이터 제작 워크벤치다.
