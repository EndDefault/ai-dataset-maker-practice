...
- 로컬 단일 사용자 RAG 기준으로 관련 근거를 찾고 파일명/chunk를 표시하면 기본 성공으로 본다.
- 여러 PDF 형식에 모두 맞는 청크 규칙을 만들기보다, 범용 구조를 유지하고 출력 모델의 흔들림을 관찰한다.
- 답변 품질은 질문 유형에 따라 75~85점 수준이면 현재 단계에서는 수용한다.
- 제외 조건, 요약의 허위 평가, 숫자 단위 오류는 출력 AI 한계로 보고 과한 코드 보정보다는 프롬프트와 평가 기준으로 관리한다.
- 예산안 같은 특정 PDF에 맞춘 출력 컬럼은 기본값으로 쓰지 않는다.
- 표 요청의 기본 컬럼은 `항목 | 핵심 내용 | 근거 파일명 | 근거 chunk`로 둔다.
...
- [x] Documents chunk 미리보기에서 섹션 묶음 형태로 후보 항목 표시
- [x] 문서 요약에서 사고 과정/근거 없는 평가를 쓰지 않도록 프롬프트 보강
